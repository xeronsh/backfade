...
[[ "$state" == "3" ]] || fail "expected CANCELLED state (3), got $state"
...
cancelled_logs=$(rpc logs --json \
  --address "$THESIS" \
  --from-block "$FROM_BLOCK" \
  --to-block "$latest" \
  'ThesisCancelled(uint64)' | jq 'length')
claimed_logs=$(rpc logs --json \
  --address "$THESIS" \
  --from-block "$FROM_BLOCK" \
  --to-block "$latest" \
  'Claimed(address,uint256)' | jq 'length')
[[ "$cancelled_logs" == "1" ]] || fail "expected one ThesisCancelled log, got $cancelled_logs"
[[ "$claimed_logs" == "$EXPECTED_CLAIMS" ]] || fail "expected $EXPECTED_CLAIMS Claimed logs, got $claimed_logs"

echo "live E2E verified: state=CANCELLED ThesisCancelled=$cancelled_logs Claimed=$claimed_logs totalClaimed=$total_claimed balance=$balance"
